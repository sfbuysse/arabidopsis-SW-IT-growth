...
#library(lme4) # for mixed modelling - not needed
#library(lmerTest) # for p value from mixed model - not needed
...
Summarize bolting time data by population to support our escape/avoidance results. The statistical analysis for this one trait is also in this section.
```{r}
# want time between emergence and bolting as the time to be analyzed
BoltingAnalysis <- Emergence %>%
  select(PotLabel, Treatment, EmergenceDate, BoltingDate) %>%
  mutate(
    Treatment = as.factor(Treatment),
    EmergenceDate = as.Date(EmergenceDate, format = "%m/%d/%Y"),
    BoltingDate = as.Date(BoltingDate, format = "%m/%d/%Y"),
    Genotype = substr(PotLabel, 1, 3)
  )

# keep only plants that emerged and survived to bolting
BoltingAnalysis <- BoltingAnalysis[complete.cases(BoltingAnalysis), ]

# add Population columns
BoltingAnalysis$Population <- "SW"
BoltingAnalysis[BoltingAnalysis$Genotype < 900, "Population"] <- "IT"
BoltingAnalysis$Population <- as.factor(BoltingAnalysis$Population)

# calculate days to bolting
BoltingAnalysis$DaysToBolting <- as.numeric(BoltingAnalysis$BoltingDate - BoltingAnalysis$EmergenceDate)

# model 
#bolt_lm <- lmer(DaysToBolting ~ Treatment * Population + (1|Genotype), data = BoltingAnalysis, contrasts = list(Treatment=contr.sum, Population = contr.sum))
#plot(fitted(bolt_lm), residuals(bolt_lm, type = "pearson", scaled = TRUE))
#summary(bolt_lm)

# we don't have replication of genotypes so the previous  model is overfit. removing the random effect does not meaningfully change the results.
bolt_lm2 <- lm(DaysToBolting ~ Treatment * Population, data = BoltingAnalysis, contrasts = list(Treatment=contr.sum, Population = contr.sum))
plot(fitted(bolt_lm2), residuals(bolt_lm2, type = "pearson", scaled = TRUE))
summary(bolt_lm2)
...
# get emmeans
#bolting_means <- as.data.frame(emmeans(bolt_lm, specs = c("Treatment", "Population")))
bolting_means2 <- as.data.frame(emmeans(bolt_lm2, specs = c("Treatment", "Population")))
```
